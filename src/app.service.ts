...
generateReviewMarkdown(commitComments: Record<string, ReviewComment[] | { summary_markdown: string }>): string {
  const lines: string[] = ['### AI Code Reviewer Thread 👍\n'];

  for (const [filePath, data] of Object.entries(commitComments)) {
    // If the AI provided a preformatted summary_markdown string, use it directly
    if (typeof data === 'object' && 'summary_markdown' in data) {
      lines.push(data.summary_markdown);
    } else {
      const comments = data as ReviewComment[];
      if (!comments?.length) continue;

      lines.push(`<details><summary>📄 <code>${filePath}</code> — ${comments.length} issue(s)</summary>\n`);

      for (const { line, comment, severity = 'Info' } of comments) {
        const emoji = this.mapSeverity(severity);
        lines.push(`- ${emoji} **(Line ${line})**: ${comment}`);
      }
  ...
  lines.push('---');
  lines.push('_Review generated by your AI bot_');
